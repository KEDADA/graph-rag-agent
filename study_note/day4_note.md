...
└───────────────────────────────────────────────────────┘
```
> 在这个开源项目的当前版本中，虽然外层起名叫“双路径”，看似是“本地 + 图谱”。但代码落地时，它实打实就是用两种不同的关键词（加没加 KB 名字缀），去搜了两次本地（Local Search）！ 并没有去搜 Global Search 的 Community。实际上是：
- 路径1：精确查询（剥离KB名称的纯问题） 
- 路径2：KB限定查询（强制带上KB名称前缀）
...
│   search("旷课处理措施")                 │
│   路径1: 纯问题 → LocalSearch("旷课...") │
│   路径2: 加前缀 → LocalSearch("华理...") │
│   → LLM评估两路结果，取最优或合并        │
│   → LLM提取: "旷课累计超过三分之一学时..."│
└──────────────────────────────────────────┘
     │
...
## 七、专有推理组件（Extra Tool Factories）

> 对应源码：`search/tool_registry.py` 中的 `EXTRA_TOOL_FACTORIES`
> 
> **一句话总结**：在基础的 6 大搜索工具之外，系统还单独注册了 3 个不继承自 `BaseSearchTool` 的**专有推理辅助工具**。它们通常不直接面对用户的 Query 提供最终答案，而是作为高级搜索（如 Deeper Research）的“重火器”挂载在内部使用。

这三个专有组件分别是：
1. **ChainOfExplorationTool**（图谱路径探索）
2. **HypothesisGeneratorTool**（假设生成器）
3. **AnswerValidationTool**（答案验证器）

---

### 7.1 Chain of Exploration（图谱路径探索）

> 对应源码：`search/tool/chain_exploration_tool.py` 及 `reasoning/chain_of_exploration.py`
>
> **一句话总结**：给定起始实体，沿着图谱的关系链一步步探索，收集路径上的实体和证据。

### 7.1 这个工具什么时候被使用？

在本项目中，Chain of Exploration **并不是拿来单打独斗的**，它作为一门“重火器”，被集成在比**深度研究（Deep Research）**更高级的**增强版深度研究（Deeper Research）**之中（源码位于 `search/tool/deeper_research_tool.py`）。

**具体使用场景：**
1. 当开启 `DeeperResearchTool` 时，系统在进行第一轮迭代前，会根据问题抽取出“焦点实体”（Focus Entities）。
2. 然后自动触发 `ChainOfExplorationSearcher.explore()` 方法。
3. 它会从这些焦点实体出发，沿着 Neo4j 图谱的边（Relationships），向外做**广度优先或特定规则的步进探索（通常限制在 2 跳 depth=2 内）**。
4. 将在这条探索路径上发现的关联实体、社区摘要（Community Summary）和文本块（Chunks），统一打包合并，作为极端丰富的上下文，喂给后文的 `ThinkingEngine` 去做推理分支。

**举个通俗的例子**：
如果用户问：“A教授和B实验室的资金违规有什么关联？”
常规的 `Local Search` 可能会直接搜名字找文本，而 `Chain of Exploration` 会在图谱里这样爬行：
`[A教授] --(隶属于)--> [某学院] --(划拨资金)--> [B实验室]`。
它把这条**关系证据链条**完整抽出来作为检索依据，这填补了纯文字向量检索在“跨跳跃逻辑推理”上的致命短板。

#### 7.1.2 与图谱搜索（GlobalSearch）的区别

```
GlobalSearch：像地图全景（鸟瞰图），找包含"旷课"的密集街区（社区），返回整个街区的概况（社区总结）。
ChainExploration：像导航路线，从A点出发沿着道路（关系）走到B点，再走到C点，收集沿途的具体风景（路径实体和关系）。
```
...
### 7.2 Hypothesis Generator（假设生成器）

> 对应源码：`search/tool/hypothesis_tool.py`
>
> **一句话总结**：在进行深度研究之前，先让大模型根据问题“脑暴”出 2-3 个可能的假设方向，避免陷入单一思维盲区。

**具体使用场景：**
在 `DeeperResearchTool` 中，如果通过算法算出来当前的用户提问**非常复杂（复杂度得分 > 0.7）**，系统就会紧急调用这个 `HypothesisGeneratorTool`。
例如，用户提问“为什么学校最近一年旷课率上升？”
系统生成的假设可能是：
- 假设1：可能是选修课考核机制变松导致的。
- 假设2：可能是学生普遍参与外部实习导致的。
随后，深度引擎就会顺着这两个并行的假设，分别开辟独立的**多分支推理（Reasoning Branches）**，从多角度发散寻找证据，最后再对这些假设进行合并与验证。

---

### 7.3 Answer Validator（答案质量验证器）

> 对应源码：`search/tool/validation_tool.py` 及 `reasoning/validator.py`
>
> **一句话总结**：交卷前的最后一道质检工序，专门看模型生成的答案是不是在“水字数”或者“胡说八道”。

**具体使用场景：**
这通常挂载在检索和生成的末端。它默认借用了 `HybridSearchTool` 的关键词提取功能。
它的核心任务是检测大模型生成的回答内容：
1. **可用性检测：** 是不是大面积包含 `I don't know`、`无法回答`、`抱歉` 等拒绝回答的废话模板？
2. **相关性检测：** 你给出的这段长篇大论，有没有真正包含之前提取出来的那些核心 Entities（实体标签）？如果在长篇大论里连目标实体的词汇都没出现几次，判定为文不对题或跑题。
在实际工作流中，如果它验证不通过，整个引擎可以直接将这轮结果打回重做或进行丢弃重试（Retry）。
...
## 十三、自测清单通俗版解答（面试/述职话术参考）

这一部分将上面的自测清单转化为了**精炼、有逻辑、适合在面试中直接拿来背诵的话术**。每一条都严格对应咱们前面笔记里梳理出的代码逻辑和系统架构设计。

### Q1：你能用自己的话解释本地搜索（LocalSearch）的三步流程吗？

**🗣️ 话术参考：**
“为了解决传统 RAG 只能字面匹配的缺陷，我们在本地搜索中设计了**‘实体检索 + 图邻居扩展’**的三步核心流程：
1. **第一步（向量匹配）：** 先用 Embedding 模型将用户查询转为向量，去 Neo4j 的向量索引里找 Top-K 个最相似的 `__Entity__`（实体节点）。这相当于先精准定位‘关键词’。
2. **第二步（图邻居扩展）：** 拿到核心实体后，我们在 Neo4j 内部执行一条强大的 Cypher 查询，以这些实体为中心，沿着边（Relationships）向外扩展 1-2 跳。把引用了这些实体的原文 Chunk、所属社区的摘要以及实体之间的内外拓扑关系全部抓出来。
3. **第三步（上下文组装）：** 最后，将抓回来的所有 Chunk、社区报告、关系链路组装成一个结构化的 JSON 上下文喂给大模型。这样 LLM 既能看到细节原文，也能看到实体间的发展脉络。”

### Q2：能说清 `retrieval_query`（本地搜索返回结果）中 5 类信息的含义和排序逻辑吗？

**🗣️ 话术参考：**
“在执行本地图谱检索的 Cypher 查询时，我们一次性会抓取并返回 5 类具有不同维度信息的上下文，并分别做了严格的排序处理来抵抗大模型的幻觉：
1. **Chunks（文本块）：** 它是最底层的原文证据。我们根据**‘该 Chunk 提到了几个召回实体’（频率）**来降序排列，提到实体越多的 Chunk 越靠前。
2. **Reports（社区摘要）：** 这是针对实体的宏观总结。我们通过 `community_rank`（层级）和自定义的 `weight`（被引用的频次）降序排列，筛选出信息密度最高的社区报告。
3. **Inside Relationships（内部关系）：** 召回实体**彼此之间**的关系，通常最直接回答了用户的问题，按权重（weight）降序排。
4. **Outside Relationships（外部关系）：** 召回实体与**非召回实体**之间的关系，用来提供补充背景，同样按权重降序排。
5. **Entities（实体描述）：** 把定位到的核心实体自身的百科描述直接带上。
这一套组合拳，确保喂给大模型的上下文既有宏观视角，又有微观依据，且高权重信息永远在最前面。”

### Q3：能解释全局搜索（GlobalSearch）为什么需要 Map-Reduce（而不是直接拼接社区摘要）吗？

**🗣️ 话术参考：**
“我们在做全局宏观问题查询（比如总结全校年度违纪趋势）时使用的是 GlobalSearch。不去直接拼接社区摘要，最核心的原因是**上下文窗口爆炸和大量噪音**。
知识图谱跑出来的 Level 0 社区可能有几十上百个，全拼在一起就算大模型吃得下，也会有严重的‘Lost in the Middle’现象，且绝大多数社区跟用户提问是毫不相干的。
因此我们引入了大数据中经典的 **Map-Reduce 范式**：
- **过滤阶段：** 先用 LLM 提取查询中的高级概念（High-level Keywords），过滤掉完全不包含这些词的底层社区。
- **Map 阶段：** 将剩下的数十个社区按批次（如 5个/批）独立喂给小模型，让它分别针对用户问题提取有用的中间报告集。
- **Reduce 阶段：** 最后把所有的中间有效报告汇集到一起，由大模型进行去重、提炼，生成最终的综合观点。
这样是以**‘多次廉价API调用的计算成本’来换取‘高质量且绝不爆显存的搜索空间’**。”

### Q4：能画出或讲出深度研究（DeepResearch）的迭代循环吗？

**🗣️ 话术参考：**
“对于‘一句话问不到底’的复杂推理问题，单纯做一次 Local 或是 Global Search 都是不够的。我们设计了 DeepResearch，它是一个**‘多轮思考-搜索-推理’**的自驱型流水线：
1. **初始化脑暴（拆解）：** 用 `QueryGenerator` 把用户的巨型复杂问题，拆成 3-4 个独立的子查询。
2. **首轮试探与双路检索（Search）：** 对于子查询，调用 `DualPathSearcher` 执行并行搜索（同时搜精确原句和带知识库前缀的变形句），交由 LLM 挑选出最好的证据结果。
3. **沉淀与反思（Think）：** `ThinkingEngine`（思考引擎）将搜回来的证据记录到推理历史日记中，并在内存里建立推理分支。
4. **缺口校验与自我追问（Evaluate & Next Round）：** 思考引擎会核对当前证据库，如果发现证据链没闭合（有信息缺口），它会立刻**自主生成新的跟进查询（Follow-up queries）**进入下一轮的 Search。
5. 直到问题闭环或抵达最大搜索设定轮数，它才调用 `AnswerValidator` 对最终答案进行质检并抛出。”

### Q5：解释你们的“两层封装架构”（工具层与组件层）的设计动机是什么？

**🗣️ 话术参考：**
“这是一个非常经典的**高内聚、低耦合**工程设计。在本项目中，搜索功能被严格劈成了两层：底层核心类和上层工具类。
1. **底层核心类（如 `LocalSearch`）：** 纯粹只干一件事——与 Neo4j 数据库通讯，写复杂的 Cypher，算相似度向量，处理 JSON 格式的字典返回。它不含有任何业务对话逻辑。
2. **上层工具封装层（如 `LocalSearchTool`）：** 它继承了 LangChain 的 `BaseTool`，戴上了 `_run` 的接口帽子。它的职责是拦截并包装，在这层之上增加了：
    - 统一的双重缓存机制和性能打点。
    - 加入了支持多轮对话上下文改写的 `history_aware_retriever`。
    - 将底层复杂的 JSON 输出转换成了大模型可以理解的标准字符串描述。
**动机总结：** 这个设计第一确保了底层搜索逻辑可以独立抽出做单元测试，不被 Agent 框架绑架；第二，未来要是把底层的图数据库从 Neo4j 换成 NebulaGraph，或者是把上层的大脑从 LangGraph 换成了 AutoGen，另一半代码完全不需要动。”
